...
namespace Tup.Cobar.Parser.Visitor
{
	...
	public sealed class MySQLOutputASTVisitor : SQLASTVisitor
	{
		...
		private void PrintList<_T0>(IList<_T0> list)
			where _T0 : ASTNode
		{
			PrintList(list, ", ");
		}
		...
		/// <param name="list">never null</param>
		private void PrintList<_T0>(IList<_T0> list, string sep)
			where _T0 : ASTNode
		{
			bool isFst = true;
		...
		{
			Tup.Cobar.Parser.Ast.Expression.Expression comparee = node.GetFirst();
			...
			Tup.Cobar.Parser.Ast.Expression.Expression start = node.GetSecond();
			...
			Tup.Cobar.Parser.Ast.Expression.Expression end = node.GetThird();
		...
		{
			Tup.Cobar.Parser.Ast.Expression.Expression comparee = node.GetOperand();
			bool paren = comparee.GetPrecedence() < node.GetPrecedence();
			if (paren)
			{
				appendable.Append('(');
		...
		public void Visit(LikeExpression node)
		{
			...
			Tup.Cobar.Parser.Ast.Expression.Expression pattern = node.GetSecond();
			...
			Tup.Cobar.Parser.Ast.Expression.Expression escape = node.GetThird();
			if (escape != null)
			{
		...
		{
			Tup.Cobar.Parser.Ast.Expression.Expression @string = node.GetString();
		...
		{
			Tup.Cobar.Parser.Ast.Expression.Expression left = node.GetLeftOprand();
			bool paren = node.IsLeftCombine() ? left.GetPrecedence() < node.GetPrecedence() : 
			...
			Tup.Cobar.Parser.Ast.Expression.Expression right = node.GetRightOprand();
			paren = node.IsLeftCombine() ? right.GetPrecedence() <= node.GetPrecedence() : right
		...
		public void Visit(PolyadicOperatorExpression node)
		{
			for (int i = 0; i < len; ++i)
			{
				...
				Tup.Cobar.Parser.Ast.Expression.Expression operand = node.GetOperand(i);
		...
		public void Visit(Trim node)
		{
			string functionName = node.GetFunctionName();
			appendable.Append(functionName).Append('(');
			Tup.Cobar.Parser.Ast.Expression.Expression remStr = node.GetRemainString();
			...
			Tup.Cobar.Parser.Ast.Expression.Expression str = node.GetString();
			str.Accept(this);
			appendable.Append(')');
		}

		public void Visit(Tup.Cobar.Parser.Ast.Expression.Primary.Function.Cast.Cast node
			)
		{
			string functionName = node.GetFunctionName();
			appendable.Append(functionName).Append('(');
			...
			Tup.Cobar.Parser.Ast.Expression.Expression info1 = node.GetTypeInfo1();
			if (info1 != null)
			{
				appendable.Append('(');
				info1.Accept(this);
				Tup.Cobar.Parser.Ast.Expression.Expression info2 = node.GetTypeInfo2();
				if (info2 != null)
				{
					appendable.Append(", ");
					info2.Accept(this);
		...
		public void Visit(GroupConcat node)
		{
			...
			PrintList(node.GetArguments());
			Tup.Cobar.Parser.Ast.Expression.Expression orderBy = node.GetOrderBy();
			if (orderBy != null)
			{
				...
				IList<Tup.Cobar.Parser.Ast.Expression.Expression> list = node.GetAppendedColumnNames
					();
		...
		{
			appendable.Append("EXTRACT(").Append(node.GetUnit().ToString()).Append(" FROM ");
			PrintList(node.GetArguments());
			appendable.Append(')');
		}
		...
		{
			appendable.Append("TIMESTAMPDIFF(").Append(node.GetUnit().ToString()).Append(", "
				);
			PrintList(node.GetArguments());
			appendable.Append(')');
		}
		...
		{
			appendable.Append("TIMESTAMPADD(").Append(node.GetUnit().ToString()).Append(", ");
			PrintList(node.GetArguments());
			appendable.Append(')');
		}
		...
		public void Visit(GetFormat node)
		{
			...
			appendable.Append(type.ToString()).Append(", ");
			PrintList(node.GetArguments());
			appendable.Append(')');
		}
		...
		public void Visit(IntervalPrimary node)
		{
			...
			Tup.Cobar.Parser.Ast.Expression.Expression quantity = node.GetQuantity();
		...
		public void Visit(CaseWhenOperatorExpression node)
		{
			...
			Tup.Cobar.Parser.Ast.Expression.Expression comparee = node.GetComparee();
			if (comparee != null)
			{
				appendable.Append(' ');
				comparee.Accept(this);
			}
			IList<Pair<Tup.Cobar.Parser.Ast.Expression.Expression, Tup.Cobar.Parser.Ast.Expression.Expression
				>> whenList = node.GetWhenList();
			foreach (Pair<Tup.Cobar.Parser.Ast.Expression.Expression, Tup.Cobar.Parser.Ast.Expression.Expression
				> whenthen in whenList)
			{
				appendable.Append(" WHEN ");
				Tup.Cobar.Parser.Ast.Expression.Expression when = whenthen.GetKey();
				when.Accept(this);
				appendable.Append(" THEN ");
				Tup.Cobar.Parser.Ast.Expression.Expression then = whenthen.GetValue();
				then.Accept(this);
			}
			Tup.Cobar.Parser.Ast.Expression.Expression elseRst = node.GetElseResult();
			if (elseRst != null)
			{
		...
		{
			Tup.Cobar.Parser.Ast.Expression.Expression parent = node.GetParent();
			if (parent != null)
			{
				parent.Accept(this);
				appendable.Append('.');
		...
		private static bool ContainsCompIn(Tup.Cobar.Parser.Ast.Expression.Expression pat
			)
		{
			if (pat.GetPrecedence() > Tup.Cobar.Parser.Ast.Expression.Expression.PrecedenceComparision)
			{
				return false;
			}
		...
		public void Visit(MatchExpression node)
		{
			...
			Tup.Cobar.Parser.Ast.Expression.Expression pattern = node.GetPattern();
		...
		public void Visit(InnerJoin node)
		{
			...
			Tup.Cobar.Parser.Ast.Expression.Expression on = node.GetOnCond();
		...
		public void Visit(GroupBy node)
		{
			...
			bool isFst = true;
			foreach (Pair<Tup.Cobar.Parser.Ast.Expression.Expression, SortOrder> p in node.GetOrderByList
				())
			{
				if (isFst)
				{
				...
				Tup.Cobar.Parser.Ast.Expression.Expression col = p.GetKey();
				col.Accept(this);
				switch (p.GetValue())
				{
					case SortOrder.Desc:
		...
		public void Visit(IndexOption node)
		{
			...
			else
			{
				if (node.GetIndexType() != null)
				{
					...
					{
						case IndexOption.IndexType.Btree:
						{
							// USING {BTREE | HASH}
						...
						case IndexOption.IndexType.Hash:
						{
		...
		public void Visit(ShowBinLogEvent node)
		{
			...
			Tup.Cobar.Parser.Ast.Expression.Expression pos = node.GetPos();
			if (pos != null)
			{
				appendable.Append(" FROM ");
				pos.Accept(this);
		...
		private void PrintLikeOrWhere(string like, Tup.Cobar.Parser.Ast.Expression.Expression
			 where)
		{
			if (like != null)
			{
		...
		public void Visit(ShowGrants node)
		{
			...
			Tup.Cobar.Parser.Ast.Expression.Expression user = node.GetUser();
			if (user != null)
			{
		...
		public void Visit(ShowProfile node)
		{
			...
			Tup.Cobar.Parser.Ast.Expression.Expression query = node.GetForQuery();
			if (query != null)
			{
		...
		public void Visit(DALSetStatement node)
		{
			...
			bool isFst = true;
			foreach (Pair<VariableExpression, Tup.Cobar.Parser.Ast.Expression.Expression> p in 
		...
		public void Visit(DMLDeleteStatement node)
		{
			...
			Tup.Cobar.Parser.Ast.Expression.Expression where = node.GetWhereCondition();
			if (where != null)
			{
				appendable.Append(" WHERE ");
				where.Accept(this);
		...
		public void Visit(DMLInsertStatement node)
		{
			...
			IList<Pair<Identifier, Tup.Cobar.Parser.Ast.Expression.Expression>> dup = node.GetDuplicateUpdate
				();
			if (dup != null && !dup.IsEmpty())
			{
				appendable.Append(" ON DUPLICATE KEY UPDATE ");
				bool isFst = true;
				foreach (Pair<Identifier, Tup.Cobar.Parser.Ast.Expression.Expression> p in dup)
				{
					if (isFst)
					{
						isFst = false;
		...
		public void Visit(DMLSelectStatement node)
		{
			...
			bool isFst = true;
			IList<Pair<Tup.Cobar.Parser.Ast.Expression.Expression, string>> exprList = node.GetSelectExprList
				();
			foreach (Pair<Tup.Cobar.Parser.Ast.Expression.Expression, string> p in exprList)
			{
				if (isFst)
				{
					isFst = false;
			...
			Tup.Cobar.Parser.Ast.Expression.Expression where = node.GetWhere();
			if (where != null)
			{
				appendable.Append(" WHERE ");
				where.Accept(this);
			...
			Tup.Cobar.Parser.Ast.Expression.Expression having = node.GetHaving();
			if (having != null)
			{
		...
		public void Visit(DMLUpdateStatement node)
		{
			...
			appendable.Append(" SET ");
			bool isFst = true;
			foreach (Pair<Identifier, Tup.Cobar.Parser.Ast.Expression.Expression> p in node.GetValues
				())
			{
				if (isFst)
				{
		...
		public void Visit(ExtDDLCreatePolicy node)
		{
			...
			appendable.Append(" (");
			bool first = true;
			foreach (Pair<int, Tup.Cobar.Parser.Ast.Expression.Expression> p in node.GetProportion
				())
			{
				if (first)
				{
