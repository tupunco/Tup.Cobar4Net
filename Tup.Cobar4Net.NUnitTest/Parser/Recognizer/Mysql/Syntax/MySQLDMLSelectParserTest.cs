...
namespace Tup.Cobar4Net.Parser.Recognizer.Mysql.Syntax
{
    ...
    public class MySQLDMLSelectParserTest : AbstractSyntaxTest
    {
        ...
        public virtual void TestSelectUnion()
        {
            ...
            Assert.AreEqual(0, select.GetFirstDistinctIndex());
            Assert.AreEqual(3, select.GetSelectStmtList().Count);
            string output = Output2MySQL(select, sql);
            Assert.AreEqual("(SELECT id FROM t1) UNION ALL (SELECT id FROM t2) UNION ALL (SELECT id FROM t3) ORDER BY d DESC LIMIT ?, 1"
                , output);
            ...
            Assert.AreEqual(1, select.GetFirstDistinctIndex());
            Assert.AreEqual(3, select.GetSelectStmtList().Count);
            output = Output2MySQL(select, sql);
            Assert.AreEqual("(SELECT id FROM t1) UNION (SELECT id FROM t2 ORDER BY id) UNION ALL (SELECT id FROM t3) ORDER BY d"
                , output);
            sql = "(select id from t1) union distInct (select id from t2) union  select id from t3";
            lexer = new MySQLLexer(sql);
            parser = new MySQLDMLSelectParser(lexer, new MySQLExprParser(lexer));
            select = (DMLSelectUnionStatement)parser.SelectUnion();
            Assert.AreEqual(2, select.GetFirstDistinctIndex());
            Assert.AreEqual(3, select.GetSelectStmtList().Count);
            output = Output2MySQL(select, sql);
            Assert.AreEqual("(SELECT id FROM t1) UNION (SELECT id FROM t2) UNION (SELECT id FROM t3)"
                , output);
        }
        ...
        public virtual void TestSelect()
        {
            ...
            Assert.IsNotNull(select);
            output = Output2MySQL(select, sql);
            Assert.AreEqual("SELECT * FROM offer AS A STRAIGHT_JOIN wp_image AS B USE KEY FOR JOIN (t1, t2) ON a.member_id = b.member_id INNER JOIN product_visit AS C WHERE a.member_id = c.member_id AND c.member_id = 'abc'", output);
            ...
            Assert.IsNotNull(select);
            output = Output2MySQL(select, sql);
            Assert.AreEqual("SELECT DISTINCTROW HIGH_PRIORITY SQL_SMALL_RESULT tb1.id, tb2.id FROM tb1, tb2 WHERE tb1.id2 = tb2.id2", output);
            ...
            Assert.IsNotNull(select);
            output = Output2MySQL(select, sql);
            Assert.AreEqual("SELECT SQL_CACHE id1, MAX(id2) FROM tb1 GROUP BY id1 HAVING id1 > 10 ORDER BY id3 DESC", output);
        ...
        public virtual void TestSelectChinese()
        {
            ...
            Assert.IsNotNull(select);
            string output = Output2MySQL(select, sql);
            Assert.AreEqual("SELECT t1.id, t2.* FROM t1, test.t2 WHERE test.t1.id = '中\\'‘文' AND t1.id = test.t2.id", output);
        }
    }
}
