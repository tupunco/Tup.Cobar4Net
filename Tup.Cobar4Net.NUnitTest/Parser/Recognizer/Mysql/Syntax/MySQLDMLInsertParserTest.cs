...
namespace Tup.Cobar4Net.Parser.Recognizer.Mysql.Syntax
{
    ...
    public class MySQLDMLInsertParserTest : AbstractSyntaxTest
    {
        ...
        public virtual void TestInsert()
        {
            ...
            Assert.AreEqual("INSERT HIGH_PRIORITY INTO test.t1 (t1.id1, id2) VALUES (?, '123')"
                , output);
            ...
            Assert.AreEqual("INSERT IGNORE INTO test.t1 (t1.id1) VALUES (?) ON DUPLICATE KEY UPDATE ex.col1 = ?, col2 = 12"
                , output);
            ...
            parser.Match(MySQLToken.Eof);
            output = Output2MySQL(insert, sql);
            Assert.AreEqual("INSERT INTO t1 VALUES (123, ?) ON DUPLICATE KEY UPDATE ex.col1 = ?"
                , output);
            ...
            parser.Match(MySQLToken.Eof);
            output = Output2MySQL(insert, sql);
            Assert.AreEqual("INSERT LOW_PRIORITY INTO t1 VALUES (0.12, 1, 2), (?), (DEFAULT)"
                , output);
            ...
            parser.Match(MySQLToken.Eof);
            output = Output2MySQL(insert, sql);
            Assert.AreEqual("INSERT LOW_PRIORITY INTO t1 SELECT id FROM t1 ON DUPLICATE KEY UPDATE ex.col1 = ?, col2 = 12"
                , output);
            ...
            parser.Match(MySQLToken.Eof);
            output = Output2MySQL(insert, sql);
            Assert.AreEqual("INSERT LOW_PRIORITY INTO t1 (t1.col1) VALUES (123), ('12\\'34')"
                , output);
            ...
            parser.Match(MySQLToken.Eof);
            output = Output2MySQL(insert, sql);
            Assert.AreEqual("INSERT LOW_PRIORITY INTO t1 (col1, t1.col2) VALUES (123, '123\\'4') ON DUPLICATE KEY UPDATE ex.col1 = ?"
                , output);
            ...
            parser.Match(MySQLToken.Eof);
            output = Output2MySQL(insert, sql);
            Assert.AreEqual("INSERT LOW_PRIORITY INTO t1 (col1, t1.col2) SELECT id FROM t3 ON DUPLICATE KEY UPDATE ex.col1 = ?"
                , output);
            ...
            parser.Match(MySQLToken.Eof);
            output = Output2MySQL(insert, sql);
            Assert.AreEqual("INSERT LOW_PRIORITY IGNORE INTO t1 (col1) SELECT id FROM t3"
                , output);
        }
    }
}
