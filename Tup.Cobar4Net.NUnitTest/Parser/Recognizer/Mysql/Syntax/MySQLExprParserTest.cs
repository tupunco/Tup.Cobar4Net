...
namespace Tup.Cobar4Net.Parser.Recognizer.Mysql.Syntax
{
    ...
    public class MySQLExprParserTest : AbstractSyntaxTest
    {
        ...
        public virtual void TestExpr1()
        {
            ...
            Assert.AreEqual(typeof(Identifier), bex.GetRightOprand().GetType(
                ));
            lexer.NextToken();
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("(N'\"abc\"abc\\'s' + 11.23 / id3) * (1E+2 - a OR b) % x'abc' AND (SELECT b'1001' ^ b'0000')"
                , output);
            ...
            Assert.AreEqual(typeof(LiteralString), ((ArithmeticAddExpression)
                bex).GetLeftOprand().GetType());
            bex = (BinaryOperatorExpression)((ArithmeticAddExpression)bex).GetRightOprand();
            Assert.AreEqual(typeof(ArithmeticDivideExpression), bex.GetType()
                );
            Assert.AreEqual(typeof(DMLSelectStatement), ((LogicalAndExpression
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("NOT ! ~ `select` IN (1, CURDATE(), `current_date`) LIKE `all` DIV a BETWEEN (c AND d) AND d | e"
                , output);
            ...
            Assert.AreEqual(typeof(LikeExpression), tex.GetFirst().GetType());
            Assert.AreEqual(typeof(LogicalAndExpression), tex.GetSecond().GetType
                ());
            Assert.AreEqual(typeof(BitOrExpression), tex.GetThird().GetType()
                );
            tex = (TernaryOperatorExpression)((BetweenAndExpression)tex).GetFirst();
            Assert.AreEqual(typeof(InExpression), tex.GetFirst().GetType());
            Assert.AreEqual(typeof(ArithmeticIntegerDivideExpression), tex.GetSecond
                ().GetType());
            bex = (BinaryOperatorExpression)(InExpression)tex.GetFirst();
            Assert.AreEqual(typeof(NegativeValueExpression), bex.GetLeftOprand
                ().GetType());
            Assert.AreEqual(typeof(InExpressionList), bex.GetRightOprand().GetType
                ());
            UnaryOperatorExpression uex = (UnaryOperatorExpression)((NegativeValueExpression)
                bex.GetLeftOprand());
            Assert.AreEqual(typeof(BitInvertExpression), uex.GetOperand().GetType
                ());
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("BINARY CASE ~ a OR b AND c ^ d XOR e WHEN 2 > ANY (SELECT a) THEN 3 ELSE 4 END IS NOT NULL = a"
                , output);
            ...
            Assert.AreEqual(typeof(ComparisionIsExpression), bex.GetLeftOprand
                ().GetType());
            Assert.AreEqual(typeof(Identifier), bex.GetRightOprand().GetType(
                ));
            ComparisionIsExpression cex = (ComparisionIsExpression)bex.GetLeftOprand();
            Assert.AreEqual(typeof(CastBinaryExpression), cex.GetOperand().GetType
                ());
            uex = (UnaryOperatorExpression)cex.GetOperand();
            Assert.AreEqual(typeof(CaseWhenOperatorExpression), uex.GetOperand
                ().GetType());
            CaseWhenOperatorExpression cwex = (CaseWhenOperatorExpression)uex.GetOperand();
            Assert.AreEqual(typeof(LogicalOrExpression), cwex.GetComparee().GetType
                ());
            ...
            Assert.AreEqual(typeof(LogicalAndExpression), bex.GetLeftOprand()
                .GetType());
            Assert.AreEqual(typeof(Identifier), bex.GetRightOprand().GetType(
                ));
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("! INTERVAL(a, b) <=> a >> b COLLATE x / ? + a != @@1 OR @var SOUNDS LIKE - (a - b) % - (d OR e)"
                , output);
            Assert.AreEqual(typeof(LogicalOrExpression), expr.GetType());
            pex = (LogicalOrExpression)expr;
            Assert.AreEqual(typeof(ComparisionNotEqualsExpression), pex.GetOperand
                (0).GetType());
            Assert.AreEqual(typeof(SoundsLikeExpression), pex.GetOperand(1).GetType
                ());
            bex = (BinaryOperatorExpression)pex.GetOperand(0);
            Assert.AreEqual(typeof(ComparisionNullSafeEqualsExpression), bex.
                GetLeftOprand().GetType());
            Assert.AreEqual(typeof(SysVarPrimary), bex.GetRightOprand().GetType
                ());
            bex = (BinaryOperatorExpression)bex.GetLeftOprand();
            Assert.AreEqual(typeof(NegativeValueExpression), bex.GetLeftOprand
                ().GetType());
            Assert.AreEqual(typeof(BitShiftExpression), bex.GetRightOprand().
                GetType());
            bex = (BinaryOperatorExpression)bex.GetRightOprand();
            Assert.AreEqual(typeof(Identifier), bex.GetLeftOprand().GetType()
                );
            Assert.AreEqual(typeof(ArithmeticAddExpression), bex.GetRightOprand
                ().GetType());
            bex = (BinaryOperatorExpression)bex.GetRightOprand();
            Assert.AreEqual(typeof(ArithmeticDivideExpression), bex.GetLeftOprand
                ().GetType());
            Assert.AreEqual(typeof(Identifier), bex.GetRightOprand().GetType(
                ));
            bex = (BinaryOperatorExpression)bex.GetLeftOprand();
            Assert.AreEqual(typeof(CollateExpression), bex.GetLeftOprand().GetType
                ());
            Assert.AreEqual(typeof(ParamMarker), bex.GetRightOprand().GetType
                ());
            bex = (BinaryOperatorExpression)((LogicalOrExpression)expr).GetOperand(1);
            Assert.AreEqual(typeof(UsrDefVarPrimary), bex.GetLeftOprand().GetType
                ());
            Assert.AreEqual(typeof(ArithmeticModExpression), bex.GetRightOprand
                ().GetType());
            bex = (BinaryOperatorExpression)bex.GetRightOprand();
            Assert.AreEqual(typeof(MinusExpression), bex.GetLeftOprand().GetType
                ());
            Assert.AreEqual(typeof(MinusExpression), bex.GetRightOprand().GetType
                ());
            uex = (UnaryOperatorExpression)bex.GetLeftOprand();
            Assert.AreEqual(typeof(ArithmeticSubtractExpression), uex.GetOperand
                ().GetType());
            uex = (UnaryOperatorExpression)bex.GetRightOprand();
            Assert.AreEqual(typeof(LogicalOrExpression), uex.GetOperand().GetType
                ());
        }
        ...
        public virtual void TestAssignment()
        {
            ...
            Assert.AreEqual(typeof(AssignmentExpression), ass.GetRightOprand(
                ).GetType());
            ass = (AssignmentExpression)ass.GetRightOprand();
            Assert.AreEqual("b", ((Identifier)ass.GetLeftOprand()).GetIdText(
                ));
            sql = "c=@var:=1";
            ...
            Assert.AreEqual("@var", usr.GetVarText());
            ...
            ass = (AssignmentExpression)ass.GetRightOprand();
            Assert.AreEqual(typeof(LogicalOrExpression), ass.GetLeftOprand().
                GetType());
            Assert.AreEqual(typeof(LogicalOrExpression), ass.GetRightOprand()
                .GetType());
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("(a := b) OR c AND (d := b'1101' OR b'01') & x'abc' ^ NULL"
                , output);
            Assert.AreEqual(typeof(LogicalOrExpression), expr.GetType());
            ...
            Assert.AreEqual(typeof(AssignmentExpression), band.GetLeftOprand(
                ).GetType());
            Assert.AreEqual(typeof(BitXORExpression), band.GetRightOprand().GetType
                ());
            ass = (AssignmentExpression)band.GetLeftOprand();
            Assert.AreEqual(typeof(LogicalOrExpression), ass.GetRightOprand()
                .GetType());
            BitXORExpression bxor = (BitXORExpression)band.GetRightOprand();
            Assert.AreEqual(typeof(LiteralHexadecimal), bxor.GetLeftOprand().
                GetType());
            Assert.AreEqual(typeof(LiteralNull), bxor.GetRightOprand().GetType
                ());
        }
        ...
        public virtual void TestLogical()
        {
            ...
            Assert.AreEqual(3, or.GetArity());
            Assert.AreEqual("b", ((Identifier)or.GetOperand(1)).GetIdText());
            sql = "a XOR b xOr c";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            ...
            Assert.AreEqual(typeof(LogicalXORExpression), xor.GetLeftOprand()
                .GetType());
            xor = (LogicalXORExpression)xor.GetLeftOprand();
            Assert.AreEqual("b", ((Identifier)xor.GetRightOprand()).GetIdText
                ());
            sql = "a XOR( b xOr c)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            ...
            Assert.AreEqual("a", ((Identifier)xor.GetLeftOprand()).GetIdText(
                ));
            Assert.AreEqual("b", ((Identifier)xor2.GetLeftOprand()).GetIdText
                ());
            Assert.AreEqual("c", ((Identifier)xor2.GetRightOprand()).GetIdText
                ());
            sql = "a and     b && c";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            ...
            Assert.AreEqual(3, or.GetArity());
            Assert.AreEqual("b", ((Identifier)and.GetOperand(1)).GetIdText());
            sql = "not NOT Not a";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            ...
            Assert.AreEqual(typeof(LogicalNotExpression), not.GetType());
            Assert.AreEqual("a", ((Identifier)not.GetOperand()).GetIdText());
        }
        ...
        public virtual void TestComparision()
        {
            ...
            Assert.AreEqual("a", ((Identifier)ba.GetFirst()).GetIdText());
            Assert.AreEqual("b", ((Identifier)ba.GetSecond()).GetIdText());
            Assert.AreEqual(false, ba.IsNot());
            ba = (BetweenAndExpression)ba.GetThird();
            Assert.AreEqual("c", ((Identifier)ba.GetFirst()).GetIdText());
            Assert.AreEqual("d", ((Identifier)ba.GetSecond()).GetIdText());
            Assert.AreEqual("e", ((Identifier)ba.GetThird()).GetIdText());
            Assert.AreEqual(true, ba.IsNot());
            sql = "a between b between c and d and e between f and g";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("a BETWEEN b BETWEEN c AND d AND e BETWEEN f AND g"
                , output);
            ...
            Assert.AreEqual("a", ((Identifier)ba.GetFirst()).GetIdText());
            Assert.AreEqual("b", ((Identifier)ba2.GetFirst()).GetIdText());
            Assert.AreEqual("c", ((Identifier)ba2.GetSecond()).GetIdText());
            Assert.AreEqual("d", ((Identifier)ba2.GetThird()).GetIdText());
            Assert.AreEqual("e", ((Identifier)ba3.GetFirst()).GetIdText());
            Assert.AreEqual("f", ((Identifier)ba3.GetSecond()).GetIdText());
            Assert.AreEqual("g", ((Identifier)ba3.GetThird()).GetIdText());
            ...
            Assert.AreEqual("a", ((Identifier)re2.GetLeftOprand()).GetIdText(
                ));
            Assert.AreEqual("b", ((Identifier)re2.GetRightOprand()).GetIdText
                ());
            Assert.AreEqual("c", ((Identifier)re.GetRightOprand()).GetIdText(
                ));
            ...
            Assert.AreEqual("a", ((Identifier)le2.GetFirst()).GetIdText());
            Assert.AreEqual("b", ((Identifier)le2.GetSecond()).GetIdText());
            Assert.AreEqual("c", ((Identifier)le.GetSecond()).GetIdText());
            Assert.AreEqual("d", ((Identifier)le.GetThird()).GetIdText());
            ...
            Assert.AreEqual("a", ((Identifier)in2.GetLeftOprand()).GetIdText(
                ));
            Assert.IsTrue(typeof(QueryExpression).IsAssignableFrom(@in.GetRightOprand
                ().GetType()));
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("a IS NOT NULL IS NOT FALSE IS NOT TRUE IS NOT UNKNOWN IS NULL IS FALSE IS TRUE IS UNKNOWN"
                , output);
            ...
            Assert.AreEqual(ComparisionIsExpression.IsUnknown, @is.GetMode());
            Assert.AreEqual(ComparisionIsExpression.IsTrue, is2.GetMode());
            Assert.AreEqual(ComparisionIsExpression.IsFalse, is3.GetMode());
            Assert.AreEqual(ComparisionIsExpression.IsNull, is4.GetMode());
            Assert.AreEqual(ComparisionIsExpression.IsNotUnknown, is5.GetMode
                ());
            Assert.AreEqual(ComparisionIsExpression.IsNotTrue, is6.GetMode());
            Assert.AreEqual(ComparisionIsExpression.IsNotFalse, is7.GetMode()
                );
            Assert.AreEqual(ComparisionIsExpression.IsNotNull, is8.GetMode());
            Assert.AreEqual("a", ((Identifier)is8.GetOperand()).GetIdText());
            ...
            Assert.AreEqual("i", ((Identifier)neq.GetRightOprand()).GetIdText
                ());
            Assert.AreEqual("h", ((Identifier)lg.GetRightOprand()).GetIdText(
                ));
            Assert.AreEqual("g", ((Identifier)l.GetRightOprand()).GetIdText()
                );
            Assert.AreEqual("f", ((Identifier)leq.GetRightOprand()).GetIdText
                ());
            Assert.AreEqual("e", ((Identifier)g.GetRightOprand()).GetIdText()
                );
            Assert.AreEqual("d", ((Identifier)geq.GetRightOprand()).GetIdText
                ());
            Assert.AreEqual("c", ((Identifier)nseq.GetRightOprand()).GetIdText
                ());
            Assert.AreEqual("b", ((Identifier)eq.GetRightOprand()).GetIdText(
                ));
            Assert.AreEqual("a", ((Identifier)eq.GetLeftOprand()).GetIdText()
                );
            ...
            Assert.AreEqual("a", ((Identifier)sl2.GetLeftOprand()).GetIdText(
                ));
            Assert.AreEqual("b", ((Identifier)sl2.GetRightOprand()).GetIdText
                ());
            Assert.AreEqual("c", ((Identifier)sl.GetRightOprand()).GetIdText(
                ));
        ...
        public virtual void TestBit()
        {
            ...
            Assert.AreEqual("d", ((Identifier)ls.GetRightOprand()).GetIdText(
                ));
            ...
            Assert.AreEqual("d", ((Identifier)xor.GetRightOprand()).GetIdText
                ());
            Assert.AreEqual("b", ((Identifier)add.GetRightOprand()).GetIdText
                ());
        }
        ...
        public virtual void TestArithmetic()
        {
            ...
            Assert.AreEqual("c", ((Identifier)mt.GetRightOprand()).GetIdText(
                ));
            Assert.AreEqual("f", ((Identifier)mi.GetOperand()).GetIdText());
            sql = "a+-b";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            ...
            Assert.IsTrue(typeof(QueryExpression).IsAssignableFrom(sub.GetLeftOprand
                ().GetType()));
            Assert.AreEqual("e", ((Identifier)sub.GetRightOprand()).GetIdText
                ());
        }
        ...
        public virtual void TestBitHex()
        {
            ...
            Assert.AreEqual("89af", ((LiteralHexadecimal)expr).GetText());
            sql = "_latin1 b'1011' ";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("_latin1 b'1011'", output);
            Assert.AreEqual("1011", ((LiteralBitField)expr).GetText());
            Assert.AreEqual("_latin1", ((LiteralBitField)expr).GetIntroducer(
                ));
            sql = "abc 0b1011 ";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
        ...
        public virtual void TestAnyAll()
        {
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("1 >= ANY (SELECT id FROM t1 LIMIT 0, 1) > ALL (SELECT tb1.id FROM tb1 AS T1, tb2 AS T2 WHERE t1.id = t2.id LIMIT 0, 1)"
                , output);
            ...
            Assert.AreEqual(typeof(LiteralNumber), ge.GetLeftOprand().GetType
                ());
            sql = "1 >= any + any";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
        ...
        public virtual void TestUnary()
        {
            ...
            Assert.AreEqual("a", ((Identifier)col.GetString()).GetIdText());
        }
        ...
        public virtual void TestUser()
        {
            ...
            Assert.AreEqual("'root'@'localhost'", usr.GetUserAtHost());
            sql = "root@localhost";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            ...
            usr = (UserExpression)expr;
            Assert.AreEqual("var@'localhost'", usr.GetUserAtHost());
        }
        ...
        public virtual void TestPrimarySystemVar()
        {
            ...
            Assert.AreEqual(VariableScope.Global, sysvar.GetScope());
            Assert.AreEqual("`all`", sysvar.GetVarText());
            sql = "@@Session . /*dd*/ any";
            lexer = new MySQLLexer(sql);
            parser = new MySQLExprParser(lexer);
            expr = parser.Expression();
            ...
            Assert.AreEqual(VariableScope.Session, sysvar.GetScope());
            Assert.AreEqual("any", sysvar.GetVarText());
            sql = "@@LOCAl . /*dd*/ `usage`";
            lexer = new MySQLLexer(sql);
            parser = new MySQLExprParser(lexer);
            expr = parser.Expression();
            ...
            sysvar = (SysVarPrimary)expr;
            Assert.AreEqual(VariableScope.Session, sysvar.GetScope());
            Assert.AreEqual("`usage`", sysvar.GetVarText());
            sql = "@@LOCAl . /*dd*/ `var1`";
            lexer = new MySQLLexer(sql);
            parser = new MySQLExprParser(lexer);
            expr = parser.Expression();
            ...
            sysvar = (SysVarPrimary)expr;
            Assert.AreEqual(VariableScope.Session, sysvar.GetScope());
            Assert.AreEqual("`var1`", sysvar.GetVarText());
            sql = "@@var1   ,";
            lexer = new MySQLLexer(sql);
            parser = new MySQLExprParser(lexer);
            expr = parser.Expression();
            ...
            sysvar = (SysVarPrimary)expr;
            Assert.AreEqual(VariableScope.Session, sysvar.GetScope());
            Assert.AreEqual("`case``1`", sysvar.GetVarText());
            lexer.NextToken();
            parser = new MySQLExprParser(lexer);
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            ...
            sysvar = (SysVarPrimary)expr;
            Assert.AreEqual(VariableScope.Session, sysvar.GetScope());
            Assert.AreEqual("_", sysvar.GetVarText());
        }
        ...
        public virtual void TestPrimary()
        {
            ...
            Assert.AreEqual(3, row.GetRowExprList().Count);
            sql = "*";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("CASE v1 WHEN `index` THEN a WHEN 2 THEN b ELSE c END"
                , output);
            CaseWhenOperatorExpression cw = (CaseWhenOperatorExpression)expr;
            Assert.AreEqual("v1", ((Identifier)cw.GetComparee()).GetIdText());
            Assert.AreEqual(2, cw.GetWhenList().Count);
            Assert.AreEqual("c", ((Identifier)cw.GetElseResult()).GetIdText()
                );
            ...
            Assert.IsNull(cw.GetComparee());
            Assert.AreEqual(1, cw.GetWhenList().Count);
            Assert.IsNull(cw.GetElseResult());
        ...
        public virtual void TestStartedFromIdentifier()
        {
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("GROUP_CONCAT(DISTINCT expr1, expr2, expr3 ORDER BY col_name1 DESC, col_name2 SEPARATOR  )"
                , output);
            sql = "GROUP_CONCAT(a||b,expr2,expr3 ORDER BY col_name1 asc,col_name2 SEPARATOR '@ ')";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("GROUP_CONCAT(a OR b, expr2, expr3 ORDER BY col_name1 ASC, col_name2 SEPARATOR @ )"
                , output);
            sql = "GROUP_CONCAT(expr1 ORDER BY col_name1 asc,col_name2 SEPARATOR 'str_val ')";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("GROUP_CONCAT(expr1 ORDER BY col_name1 ASC, col_name2 SEPARATOR str_val )"
                , output);
            sql = "GROUP_CONCAT(DISTINCT test_score ORDER BY test_score DESC )";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("GROUP_CONCAT(DISTINCT test_score ORDER BY test_score DESC SEPARATOR ,)"
                , output);
            sql = "GROUP_CONCAT(DISTINCT test_score ORDER BY test_score asc )";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("GROUP_CONCAT(DISTINCT test_score ORDER BY test_score ASC SEPARATOR ,)"
                , output);
            sql = "GROUP_CONCAT(c1)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
        ...
        public virtual void TestInterval()
        {
            ...
            Tup.Cobar4Net.Parser.Ast.Expression.Expression expr = parser.Expression();
            string output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('2009-01-01', INTERVAL (6 / 4) HOUR_MINUTE)"
                , output);
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('2100-12-31 23:59:59', INTERVAL '1:1' MINUTE_SECOND)"
                , output);
            sql = "DATE_SUB('2005-01-01 00:00:00',INTERVAL '1 1:1:1' DAY_SECOND)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_SUB('2005-01-01 00:00:00', INTERVAL '1 1:1:1' DAY_SECOND)"
                , output);
            sql = "DATE_ADD('1900-01-01 00:00:00',INTERVAL '-1 10' DAY_HOUR)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('1900-01-01 00:00:00', INTERVAL '-1 10' DAY_HOUR)"
                , output);
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('1992-12-31 23:59:59.000002', INTERVAL '1.999999' SECOND_MICROSECOND)"
                , output);
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('1992-12-31 23:59:59.000002', INTERVAL '1:1.999999' MINUTE_MICROSECOND)"
                , output);
            sql = "DATE_ADD('1992-12-31 23:59:59.000002',INTERVAL '1:1:1.999999' hour_MICROSECOND)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('1992-12-31 23:59:59.000002', INTERVAL '1:1:1.999999' HOUR_MICROSECOND)"
                , output);
            sql = "DATE_ADD('2100-12-31 23:59:59',INTERVAL '1:1:1' hour_SECOND)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('2100-12-31 23:59:59', INTERVAL '1:1:1' HOUR_SECOND)"
                , output);
            sql = "DATE_ADD('1992-12-31 23:59:59.000002',INTERVAL '1 1:1:1.999999' day_MICROSECOND)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('1992-12-31 23:59:59.000002', INTERVAL '1 1:1:1.999999' DAY_MICROSECOND)"
                , output);
            sql = "DATE_ADD('2100-12-31 23:59:59',INTERVAL '1 1:1' day_minute)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('2100-12-31 23:59:59', INTERVAL '1 1:1' DAY_MINUTE)"
                , output);
            sql = "DATE_ADD('2100-12-31',INTERVAL '1-1' year_month)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("DATE_ADD('2100-12-31', INTERVAL '1-1' YEAR_MONTH)"
                , output);
        ...
        public virtual void TestMatchExpression()
        {
            ...
            Tup.Cobar4Net.Parser.Ast.Expression.Expression expr = parser.Expression();
            string output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (title, body) AGAINST ('database' WITH QUERY EXPANSION)"
                , output);
            Assert.AreEqual("MATCH (title, body) AGAINST ('database' WITH QUERY EXPANSION)"
                , output);
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (title, body) AGAINST ((abc IN (d)) IN BOOLEAN MODE)"
                , output);
            ...
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (col1, col2, col3) AGAINST (a := b := c IN BOOLEAN MODE)"
                , output);
            sql = "MATCH (title,body) AGAINST ((a and (b ||c)) IN boolean MODE)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (title, body) AGAINST (a AND (b OR c) IN BOOLEAN MODE)"
                , output);
            sql = "MATCH (title,body) AGAINST ((a between b and c) IN boolean MODE)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (title, body) AGAINST (a BETWEEN b AND c IN BOOLEAN MODE)"
                , output);
            sql = "MATCH (title,body) AGAINST ((a between b and (abc in (d))) IN boolean MODE)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (title, body) AGAINST ((a BETWEEN b AND abc IN (d)) IN BOOLEAN MODE)"
                , output);
            sql = "MATCH (title,body) AGAINST ((not not a) IN boolean MODE)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (title, body) AGAINST (NOT NOT a IN BOOLEAN MODE)"
                , output);
            sql = "MATCH (title,body) AGAINST ((a is true) IN boolean MODE)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (title, body) AGAINST (a IS TRUE IN BOOLEAN MODE)"
                , output);
            sql = "MATCH (title,body) AGAINST ((select a) IN boolean MODE)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (title, body) AGAINST (SELECT a IN BOOLEAN MODE)"
                , output);
            sql = "MATCH (title,body) AGAINST ('database' IN NATURAL LANGUAGE MODE)";
            parser = new MySQLExprParser(new MySQLLexer(sql));
            expr = parser.Expression();
            output = Output2MySQL(expr, sql);
            Assert.AreEqual("MATCH (title, body) AGAINST ('database' IN NATURAL LANGUAGE MODE)"
                , output);
